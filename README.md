...
├── patricio_escondite/
├── patricio_interfaces/
├── patricio_voz/             ← Reconocimiento de voz (STT)
...
- Micrófono conectado (para reconocimiento de voz)
...
```bash
cp .env.example .env
# Revisar: MYSQL_PASSWORD=Patricio_local1!
```

### Entorno virtual Python (API web)
```bash
...
### 🎙️ Reconocimiento de voz (STT)

Para verificar el micrófono y el sistema de reconocimiento de voz:

```bash
# Terminal 1 — Lanzar nodo de voz
source /opt/ros/jazzy/setup.bash
source ~/turtlebot3_ws/install/setup.bash
ros2 launch patricio_voz voice_stt.launch.py
# Esperar ~2 s sin hablar (calibración automática)

# Terminal 2 — Monitorizar estado de voz
source ~/turtlebot3_ws/install/setup.bash
ros2 topic echo /patricio/voice_status

# Terminal 3 — Monitorizar entrada de voz
source ~/turtlebot3_ws/install/setup.bash
ros2 topic echo /patricio/voice_input
```

Con el micrófono cerca, habla con claridad (ej.: *«¿Qué hora es?»*) y espera 5–10 s para ver la respuesta en los Terminales 2 y 3.
...
# Terminal 4 — API Flask (con entorno virtual)
cd ~/turtlebot3_ws/src/patricio/patricio_web
...
Credenciales por defecto: `admin@patricio.local` / `1234`
...
### 🗄️ Problema 12: Error de conexión a la base de datos
Verificar que MySQL está activo:
```bash
sudo systemctl status mysql
```
Comprobar que el archivo `.env` existe y contiene las credenciales correctas (`MYSQL_PASSWORD=Patricio_local1!`).  
Probar la conexión directamente:
```bash
curl -s http://127.0.0.1:5000/api/db/health | python3 -m json.tool
```

### 🎙️ Problema 13: El micrófono no detecta voz
Verificar que el nodo de voz está corriendo:
```bash
ros2 launch patricio_voz voice_stt.launch.py
```
- Esperar los ~2 s de calibración antes de hablar
- Comprobar que el micrófono está conectado y reconocido por el sistema
- Verificar que los tópicos `/patricio/voice_status` y `/patricio/voice_input` están activos:
```bash
ros2 topic echo /patricio/voice_status
```
