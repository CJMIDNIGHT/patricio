#!/usr/bin/env python3
"""Genera PDF de la guía STT con estilo Patricio."""
...
MD_PATH = PKG_DIR / 'GUIA_INSTALACION_Y_PRUEBAS.md'
OUT_PATH = PKG_DIR / 'GUIA_INSTALACION_Y_PRUEBAS.pdf'
...
def _draw_header_footer(canvas, doc):
    ...
    if canvas.getPageNumber() == 1:
        ...
        canvas.drawString(2 * cm, 1.2 * cm, 'Patricio — Reconocimiento de voz (STT)')
...
def _cover_page(styles) -> list:
    ...
    flow.append(
        Paragraph(
            'Guía de instalación y pruebas<br/>Reconocimiento de voz (Speech-to-Text)',
            styles['cover_title'],
        )
    )
    ...
    flow.append(
        Paragraph(
            'Paquete <b>patricio_voz</b> · Micrófono Konobo USB · ROS 2',
            styles['cover_sub'],
        )
    )
