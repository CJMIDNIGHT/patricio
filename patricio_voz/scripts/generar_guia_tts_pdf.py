#!/usr/bin/env python3
"""Genera PDF de la guía TTS y pantalla con estilo Patricio."""
...
MD_PATH = PKG_DIR / 'GUIA_TTS_y_PANTALLA.md'
OUT_PATH = PKG_DIR / 'GUIA_TTS_y_PANTALLA.pdf'
...
def _draw_header_footer(canvas, doc):
    ...
    if canvas.getPageNumber() == 1:
        ...
        canvas.drawString(2 * cm, 1.2 * cm, 'Patricio — TTS y pantalla (T18)')
...
def _cover_page(styles) -> list:
    ...
    flow.append(
        Paragraph(
            'Guía de instalación y pruebas<br/>Text-to-Speech y visualización en pantalla',
            styles['cover_title'],
        )
    )
    ...
    flow.append(
        Paragraph(
            'Paquete <b>patricio_voz</b> · T18 · ROS 2 · pyttsx3 / gTTS',
            styles['cover_sub'],
        )
    )
